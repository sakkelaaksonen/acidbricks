...
var initGame = function() {
    ...
    //restart
    document.querySelector('#restart').addEventListener('click',restartBtn,false);
...
pj0ng.prototype.init = function() {
    ...
    for(i = 0; i<this.brickRows; i++) {
        ...
        for(j = 0; j<this.brickCols;j++) {
			...
			spY = Math.floor(Math.random() * (this.brickSpriteBottom - this.brickH))+this.paddleH;
...
pj0ng.prototype.drawDashboard = function() {
	...
	if(this.debug) {
        ...
        this.ctx.fillText  ('SnaikTopBottom:'+this.snaikBB.t +  ' '+this.snaikBB.b,10, this.canvas.height- 120);
		this.ctx.fillText  ('SnaikLeftRiht:'+this.snaikBB.l +  ' '+this.snaikBB.r,10, this.canvas.height- 100);
...
pj0ng.prototype.drawAll = function() {	
	...
	if (this.snaikIsRunning) {
    	...
    	if (Math.floor(Math.random() * this.snaikSeed +1) == 1) this.snaikIsRunning = 1;
	}
  //detect brick collision
	...
	if (this.ballY < this.brickRows * rowH && row >= 0 && col >= 0 && this.bricks[row][col].hitcount >0) {
	   this.dY = -this.dY;
	   this.bricks[row][col].hitcount--;
	   if(!this.bricks[row][col].hitcount)
	       this.brickCount--;
	}
	...
	if
	(
		(this.ballX < this.snaikBB.r && this.ballX > this.snaikBB.l) 
		&&
		(this.ballY < this.snaikBB.b && this.ballY > this.snaikBB.t) 
	)  
	{
	...
	if (this.ballX + this.dX > this.xLimit || this.ballX + this.dX < this.ballRadius)
        this.dX = -this.dX;
	...
	//if ball is about to hit bottom limit
        ...
        if (this.ballX > this.paddleX && this.ballX < this.paddleX + this.paddleW) {
			this.dX = Math.floor(15 * ((this.ballX-(this.paddleX+this.paddleW/2))/this.paddleW));
			//
			//console.log(this.dX);
			this.dY = -this.dY;
		} else {
